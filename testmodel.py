...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=150, in_channels=3, input_size=112, input_len=16, n_iter=3,
                 learnable=[1, 1, 1, 1, 1], config=0, dropout=0.3):
        self.inplanes = 64
    ...
    def forward(self, x):
        # x is BxCxTxHxW
        print("Lets check what goes on in here")
        print("size of x", x.size())
        b, c, t, h, w = x.size()

        x = self.conv1(x)
        print("Impact of Conv1", x.size())
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        print("Impact of Normal Convulations", x.size())
        x = self.layer1(x)
        x = self.layer2(x)
        print("Impact of two resnet layers, first outputs 64 channels and second 128", x.size())

        _, ci, t, h, w = x.size()
        res = F.avg_pool3d(x, (3, 1, 1), 1, 0)  # x[:,:,1:-1].contiguous()
        print("Impact of a avg pool layer", res.size())

        x = self.flow_cmp(x)
        print("Impact of a flow layer convulution", x.size())
        x = self.flow_layer.norm_img(x)
        print("Impact of a flow layer norm", x.size())
        # handle time
        _, c, t, h, w = x.size()
        t = t - 1
...
def resnet18(pretrained=False, mode='rgb', **kwargs):
